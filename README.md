Assembly Theory - Formalizing Assembly Spaces, Discovering Patterns and Bounds
Wawrzyniec Bieniawski, Andrzej Tomski, Szymon £ukaszyk, Piotr Masierak, Szymon Tworz
Preprint:
https://www.preprints.org/manuscript/202409.1581
Publication: #pending#
Royal Society Open Science - Manuscript ID RSOS-260082

b - alphabet size
N - string length
ASI - assembly index

List of files:
--------------------------------------------------------
A003313.xlsx
Length of shortest addition chain for N (OEIS A003313) and the minimum assembly index of strings for all b, illustrating particular pathways shown in Tables 1 & 2 and discussed in Section 3 of the paper.
--------------------------------------------------------
Plans_b2
Assembly plans of max ASI strings for b=2 (Tables 3 & 4 of the paper).
--------------------------------------------------------
Plans_b3
Assembly plans of max ASI strings for b=3  (Table 5 of the paper).
--------------------------------------------------------
Plans_b4
Assembly plans of max ASI strings for b=4  (Table 6 of the paper).
--------------------------------------------------------
Figure10.xls
Data for Figure 10 and Table 7 of the paper:
"The ASI (a), Constrained ASI (b), Re-Pair steps (c), and LZW steps (d) of all bitstrings of length N=10, as a function of their decimal representations ({0, 1, . . . , 1023}), in squares with sides equal to 32 pixels..." 
--------------------------------------------------------
allN_8-16.xls
Lists of all bitstrings of length 8<=N<=16 along with their ASI.
--------------------------------------------------------
QuickASISolver.exe
A Windows console program to calculate the string ASI.
--------------------------------------------------------
